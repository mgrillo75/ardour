...
function factory() return function()
  ...
  while i <= #onsets do
    ...
    local group_onsets = {onsets[i]}
    local j = i + 1
    while j <= #onsets and onsets[j].time - onsets[j-1].time <= config.slice_duration do
      table.insert(group_onsets, onsets[j])
      j = j + 1
    end
    local slice = {
      index = slice_idx,
      onset_time = group_onsets[1].time,
      onset_frame = group_onsets[1].frame,
