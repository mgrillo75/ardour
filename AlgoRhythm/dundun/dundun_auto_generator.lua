...
function factory() return function()
  ...
  local config = {
    slice_duration = 0.3,      -- Duration of each hit region in seconds
    ...
    onset_threshold = 0.02,    -- Energy threshold for onset detection
    min_onset_interval = 0.05, -- Minimum time between onsets
  ...
  print("Created " .. #hit_slices .. " hit slice candidates")
  ...
  -- Select a single fixed representative from each tone group
  local tone_a_hit = tone_a_hits[1]
  local tone_b_hit = tone_b_hits[1]

  if not tone_a_hit or not tone_b_hit then
    print("ERROR: Could not select representative hits for pattern.")
    return
  end

  print(string.format("Using fixed tone A: slice %d at %.3fs", tone_a_hit.index, tone_a_hit.onset_time))
  print(string.format("Using fixed tone B: slice %d at %.3fs", tone_b_hit.index, tone_b_hit.onset_time))

  -- Build deterministic rhythmic sequence using only fixed A/B hits
  for rep = 1, config.repeat_count do
    for pos, pattern_element in ipairs(config.pattern) do
      ...
      if pattern_element == 1 then
        selected_hit = tone_a_hit
        tone_type = "A"
      elseif pattern_element == 2 then
        selected_hit = tone_b_hit
        tone_type = "B"
      end
